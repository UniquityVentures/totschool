...
func pluginRoutes() lamu.PluginFeatures[lamu.Route] {
	return lamu.PluginFeatures[lamu.Route]{
		Entries: []registry.Pair[string, lamu.Route]{
			{Key: "clients.DashboardRoute", Value: lamu.Route{Path: AppUrl, Handler: lamu.NewDynamicView("clients.DashboardView")}},
			{Key: "clients.ListRoute", Value: lamu.Route{Path: AppUrl + "list/", Handler: lamu.NewDynamicView("clients.ListView")}},
